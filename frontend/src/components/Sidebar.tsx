    ...
    <>
      ...
      {isMobileMenuOpen && (
        <button
          ...
          className="lg:hidden fixed inset-0 bg-black/50 z-[50] cursor-default w-full h-full border-none outline-none focus:outline-none"
      ...
      <div className="p-4 border-t border-color-border space-y-2 bg-bg-tertiary/20">
        {user ? (
          <div className="px-4 py-2 flex items-center gap-3 mb-2">
             <div className="w-8 h-8 rounded-lg bg-bg-tertiary border border-color-border flex items-center justify-center overflow-hidden">
                {user.profile_picture_url ? (
                   ...
                   <span className="text-[10px] font-black uppercase text-text-primary">
                      ...
                      {user.last_name ? user.last_name.charAt(0) : user.username?.charAt(1)}
                   </span>
                )}
             </div>
             <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-text-tertiary font-bold truncate">{user.email}</p>
             </div>
          </div>
        ) : (
          <div className="px-4 py-2 flex items-center gap-3 mb-2 animate-pulse">
             <div className="w-8 h-8 rounded-full bg-bg-tertiary border border-color-border"></div>
             <div className="flex-1 space-y-2">
                <div className="h-3 bg-bg-tertiary rounded w-20"></div>
                <div className="h-2 bg-bg-tertiary rounded w-24"></div>
             </div>
          </div>
        )}
