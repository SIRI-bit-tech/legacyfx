...
async def get_platform_stats(db: AsyncSession = Depends(get_db), _ = Depends(require_admin)):
    ...
    pending_kyc = select(func.count(User.id)).where(User.kyc_status == KYCStatus.PENDING)
    
    p_dep = (await db.execute(pending_dep)).scalar() or 0
    p_with = (await db.execute(pending_with)).scalar() or 0
    p_kyc = (await db.execute(pending_kyc)).scalar() or 0
...
async def verify_user_kyc(
    ...
    await db.commit()
    return {"message": f"User KYC status updated to {request.status}"}
