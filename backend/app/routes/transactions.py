...
async def recent_transactions(
    ...
    dep_stmt = select(Deposit).where(Deposit.user_id == userId).order_by(Deposit.created_at.desc()).limit(limit)
    with_stmt = select(Withdrawal).where(Withdrawal.user_id == userId).order_by(Withdrawal.created_at.desc()).limit(limit)
    trade_stmt = select(ExecutionTrade).where(ExecutionTrade.user_id == userId).order_by(ExecutionTrade.created_at.desc()).limit(limit)
    ...
    for d in deposits:
        combined.append({
            "type": "deposit", "asset": d.asset_symbol, "amount": d.amount,
            "status": map_deposit_status(d.status), "network": d.blockchain_network or "On-chain",
            "date": d.created_at.isoformat() if d.created_at else None,
            "txHash": d.transaction_hash or "", "_created": d.created_at,
        })

    for w in withdrawals:
        combined.append({
            "type": "withdraw", "asset": w.asset_symbol, "amount": w.amount,
            "status": map_withdraw_status(w.status), "network": w.blockchain_network or "On-chain",
            "date": w.created_at.isoformat() if w.created_at else None,
            "txHash": w.transaction_hash or "", "_created": w.created_at,
        })

    for t in trades:
        combined.append({
            "type": "trade", "asset": t.symbol, "amount": t.quantity, "status": "filled",
            "network": "Internal", "date": t.created_at.isoformat() if t.created_at else None,
            "txHash": "", "_created": t.created_at,
        })
    ...
    return {
        "transactions": [
            {
                "type": i["type"], "asset": i["asset"], "amount": i["amount"],
                "status": i["status"], "network": i["network"], "date": i["date"],
                "txHash": i["txHash"],
            }
            for i in items
        ]
